  ...
  return (
    <div className="flex items-center justify-between p-6 bg-gray-900/50 backdrop-blur-sm border-b border-gray-700/50">
      <div className="flex items-center space-x-4">
        <button onClick={() => setSelectedFriend(null)} className="p-2 rounded-full hover:bg-gray-800/50 transition-all duration-300">
        ...
        <div className="flex items-center space-x-3">
          <div className="relative">
            <img src={selectedFriend?.profileImage || "/default-avatar.png"} alt="Profile" className="w-12 h-12 rounded-full border-2 border-gray-700" />
            {isUserOnline(selectedFriend?._id) && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-gray-900"></div>}
          </div>
          <div>
            <h2 className="text-lg font-bold text-white">{selectedFriend?.username}</h2>
            <p className="text-sm text-gray-400">{isUserOnline(selectedFriend?._id) ? 'Online' : 'Offline'}</p>
          </div>
        </div>
      </div>
